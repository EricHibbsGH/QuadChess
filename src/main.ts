...
import { COLOR_NAMES, type PieceType } from './engine/pieces.js';
import {
  legalDestinations,
  playersInCheck,
  type GameEvent,
...
import type { MoveIntent } from './multiplayer/transport.js';
import {
  clearGame,
  loadGame,
  loadPreferences,
...
class App {
  ...
  constructor() {
    ...
    // through this handler, so adding a networked transport changes nothing else.
    this.#transport.onRemote((intent) => this.#applyIntent(intent));
  ...
  async #activate(coord: Coord): Promise<void> {
    ...
    if (piece && piece.owner === state.toMove && !piece.dead) {
      this.#selected = coord;
  ...
  async #newGame(): Promise<void> {
    ...
    this.#session.reset(this.#session.profile);
    this.#resetViewState();
    this.#announcer.say(`New game. ${describeTurn(this.#session.state.toMove, false)}`);
  }
  ...
  async #changeProfile(profileId: string): Promise<void> {
    ...
    this.#resetViewState();
    this.#announcer.say(`${profile.label}. ${profile.description}`);
  }
